...
function addTemplate(templateUrl) {
  if (templateUrl) {
    ...
    // 显示操作提示信息
    console.log(chalk.yellow('操作提示：请输入模板仓库链接'));
    console.log('');
...
function processTemplate(templateUrl) {
  ...
  console.log(chalk.blue(`正在添加模板: ${templateName}`));
  ...
  if (existingTemplateIndex >= 0) {
    ...
    console.log(chalk.yellow(`模板 ${templateName} 已存在，已更新配置`));
  } else {
  ...
  console.log(chalk.green('模板添加完成！'));
}
