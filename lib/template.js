...
function cleanTemplateFiles(templatePath) {
  try {
    ...
    files.forEach((file) => {
      ...
      // 删除不必要的文件和目录
      if ((!stat.isDirectory() && (file === '.gitignore' || file === 'LICENSE' || file === 'README.md')) || (stat.isDirectory() && file === '.git')) {
...
function downloadTemplate(templateUrl, templateName) {
  ...
  try {
    ...
    console.log(chalk.yellow('提示: 尝试使用HTTPS格式的链接，例如: https://github.com/username/template-repo.git'));
    process.exit(1);
  }
}
